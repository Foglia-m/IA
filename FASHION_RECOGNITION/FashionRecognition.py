...
def defineModel(input_shape, num_classes):
    ...
    model.add(Conv2D(64, (3, 3), input_shape=input_shape, kernel_initializer='he_uniform', padding='same', activation='relu'))
    ...
    # if you finetune a pretrained: you should use a bigger learning rate that decreases through time
...
def main():
    ...
    #displaying first images:
    #todisplay = trainX[0]
    #for i in range (1,5):
    #    todisplay= np.concatenate((todisplay,trainX[i]),axis=1)
    #cv2.imshow("images",todisplay)
    #cv2.waitKey(0)
